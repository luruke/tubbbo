...
// TODO use pointer velocity/direction to push noise tubes.
...
class Pointer {
  ...
  onStart(event) {
    ...
    bidello.trigger({ name: 'pointerStart' }, {
      pointer: this,
    });
  }
  ...
  onMove(event) {
    ...
    bidello.trigger({ name: 'pointerMove' }, {
      pointer: this,
    });

    if (this.isTouching) {
      bidello.trigger({ name: 'pointerDrag' }, {
        pointer: this,
      });
    }
  }
  ...
  onEnd() {
    ...
    bidello.trigger({ name: 'pointerEnd' }, {
      pointer: this,
    });
  }
}
